...
int licznik = 0;
int pole = 0, pole_1 = 0, pole_2 = 0, i_zas1 = 0, i_zas2 = 0, j_zas1 = 0, j_zas2 = 0, pocz_i = 0, pocz_j = 0, spr = 0;
int pkt_g = 0, pkt_k = 0;
...
void ruchy(char tab_mem[][4], char tab_mem_znak[][4], int tab_pole[][4], char odkryte_pola[][4], int wolne_pola[]) {
	...
	system("cls");
	tablica_wynikow_memory(pkt_g, pkt_k);
}
		...
		do {
			...
			for (int i = 0; i < 16; i++)
				if (pole_2 == wolne_pola[i])
					spr = i;
		} while ((wolne_pola[spr] == pole_2) || (pole_1 == pole_2));
		cout << pole_2 << endl;
